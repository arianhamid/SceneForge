...
هر کلاسی که _به‌طور طبیعی_ متدهای `name` و `build` را داشته باشد، «یک KnowledgeBuilder محسوب می‌شود» — حتی اگر هیچ‌وقت صریحاً از `KnowledgeBuilder` ارث‌بری نکرده باشد. این شبیه به این است که بگوییم «اگر مثل اردک راه برود و مثل اردک صدا کند، اردک است» — نه این‌که حتماً باید گواهی «من اردک هستم» داشته باشد.
